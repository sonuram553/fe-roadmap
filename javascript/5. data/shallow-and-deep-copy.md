...
function deepCopy(val) {
  ...
  for (const key in val) {
    if (Object.hasOwn(val, key)) { // for...in iterates inherited prototype properties too, so guard with hasOwn
      objectCopy[key] = deepCopy(val[key]);
    }
  }
